...
# =============================================================================
# lockcart&lib.sh
# Script de verrouillage/déverrouillage des dossiers des cartes Arduino et des librairies de l'appli en cours
# Protège les cartes et librairies contre les mises à jour involontaires
# Script apppelé directement par l'utilisateur
# =============================================================================
...
if [ -z "$RAW_DATA_PATH" ]; then
    notify_user "Erreur" "lockcart&lib.sh : Chemin YAML introuvable"
    exit 1
fi
...
# Le dossier des librairies est toujours le sous-dossier "libraries"
...
lock() {
    ...
    # On ne verrouille les librairies que si le dossier existe
    # (il peut ne pas exister si le projet n'en utilise pas)
    if [ -d "$LIB_DIR" ]; then
        chmod -R a-w "$LIB_DIR"
    fi

    notify_user "Arduino" "🔒 Dossiers cartes et librairies verrouillés"
}
...
unlock() {
    ...
    notify_user "Arduino" "🔓 Dossiers cartes et librairies déverrouillés"
}
...
# - Si le dossier cartes est actuellement accessible en écriture → on verrouille cartes et libs
# - Si le dossier cartes est déjà verrouillé → on déverrouille cartes et libs
#    => c'est donc les dossier des cartes qui fait référence pour connaître l'état.
